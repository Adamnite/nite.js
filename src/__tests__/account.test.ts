...
test('createAccount', () => {
  match(
    createAccount(), {
      ok: v => {
        expect(v.publicKey.startsWith('0x'));
        expect(v.privateKey.startsWith('0x'));

        expect(v.address.length).toBe(28);
        expect(v.publicKey.length).toBe('0x'.length + 130);
...
test('getAccountFromPrivateKey', () => {
  ...
  const address = '23m3Ho7PwouaFzU8iXMLygwuXNW7';
  const publicKey = '0x04c205aa76174a126606bc6f411a1ee421e6c2219d4af8353f1a8b6ca359d796b7de2e5fb84c87a806dc40bcd30cda66712548c69b9779b58da9020a7342128a5f';
  ...
  match(
    getAccountFromPrivateKey(privateKey), {
      ok: v => {
        ...
        expect(v.publicKey).toBe(publicKey);
